...
/**
 *API: GET /admin/category  含义:客户端获取所有菜品类别，按菜品类别编号升序排列
 */
router.get('/', (req, res) => {
...
/**
 *API: DELETE /admin/category/:cid  含义:根据表示菜品编号的路由参数，删除该菜品  返回值形如{code:200,msg:'1 category deleted'}、{code:400,msg:'0 category deleted'}
 */
router.delete('/:cid', (req, res) => {
    //注意：删除菜品之前必须先把属于该类菜品的类别编号设置为NULL
...
/**
 *API: POST /admin/category 请求参数：{cname:'xxx'}  含义:添加新的菜品类别 返回值形如：{code:200,msg:'1 category added'}
 */
router.post('/',(req,res)=>{
...
/**
 *API: PUT  /admin/category  请求参数：{cid:xx,cname:'xxx'}  含义：根据菜品编号修改该类别  返回值形如：{code:200,msg:'1 category modified'}、{code:400,msg:'0 category modified,not exists'}、{code:401,msg:'0 category modified,no modification'}
 */
router.put('/',(req,res)=>{
